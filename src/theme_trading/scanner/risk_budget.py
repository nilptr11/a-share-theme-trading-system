...
def risk_budget_for_plan(
    ...
    if score >= SCORE_STRONG:
        pct = 0.01
        reasons = ["市场评分 ≥ 7，标准风险预算 1%"]
    elif score == SCORE_MID:
        pct = 0.005
        reasons = ["市场评分 = 6，轻仓风险预算 0.5%"]
    else:
        pct = 0.0
        reasons = ["市场评分 < 6，不开新仓"]
    ...
    if plan_type == "trial" and pct > 0:
        pct = min(pct, 0.005)
        reasons.append("主线未确认买点一试错")

    if emotion_extreme and pct > 0:
        pct = min(pct, 0.005)
        reasons.append("情绪极端或板块高潮")

    if buy_point_name == "买点四_趋势均线" and pct > 0:
        pct = pct / 2
        reasons.append("买点四趋势成熟，风险预算按当前档位减半")

    return {
        "risk_budget_pct": pct,
        "risk_budget_label": f"{pct:.2%}",
        "risk_budget_reason": "；".join(reasons),
    }
