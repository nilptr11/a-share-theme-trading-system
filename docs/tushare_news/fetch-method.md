...
```bash
uv run scripts/fetch_tushare_news.py --all
```
...
```bash
uv run scripts/fetch_tushare_news.py --all --run-id 2026-05-25-1955
```

只抓取指定来源：

```bash
uv run scripts/fetch_tushare_news.py --source eastmoney --source sina
```
